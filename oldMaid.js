...
class Card extends Obj{
  ...
  handCard(){
    ...
    for(num = 0; num < player.length; num++){
      push();
      translate(width / 2 + (( -40 * (player.length - num)) / 2), height / 3);
  ...
  update(){
    ...
    for(num = 0; num < player.length; num++){
      push();
      translate(this.pos.x+(( -20 * (player.length - num)) / 2 ), this.pos.y);
      rotate(this.rotation);
      //image(cardImg[player[num]], 0, 0, this.size, 2 * this.size);
    ...
    for(num = 0; num < opponent.length; num++){
      ...
      translate(others[0].x + (( -20 * (opponent.length - num)) / 2 ), others[0].y);
      rotate(this.rotation);
